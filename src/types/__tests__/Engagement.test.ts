...
describe('isStartWindowOpen', () => {
  ...
  it('respects a different lead time', () => {
    ...
    expect(isStartWindowOpen(scheduledStart, now, 30)).toBe(false);
    expect(isStartWindowOpen(scheduledStart, now, 60)).toBe(true);
  });

  it('defaults the lead time to WORK_START_LEAD_MINUTES', () => {
    const atBoundary = new Date(
      scheduledStart.getTime() - WORK_START_LEAD_MINUTES * 60_000,
    );
    const beforeBoundary = new Date(atBoundary.getTime() - 60_000);
    expect(isStartWindowOpen(scheduledStart, atBoundary)).toBe(true);
    expect(isStartWindowOpen(scheduledStart, beforeBoundary)).toBe(false);
  });
});
